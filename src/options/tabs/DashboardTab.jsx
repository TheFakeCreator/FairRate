...
export default function DashboardTab({
  ...
  const handleDeleteRating = async (movieId) => {
    if (
      confirm(
        "Are you sure you want to delete this rating? This will remove it from the cloud as well.",
      )
    ) {
  ...
  const handleImportCSV = (e) => {
    ...
    reader.onload = async (event) => {
      ...
      if (constIdx === -1 || ratingIdx === -1 || titleIdx === -1) {
        alert(
          "Could not find required columns (Const, Your Rating, Title) in CSV.",
        );
        return;
      }
      ...
      const importedCount = await batchImportIMDbRatings(
        parsedRatings,
        (percent, count) => {
  ...
  const filteredRatings = ratings
    .filter((r) => {
      ...
      if (
        filterPresetId !== "all" &&
        (r.presetId || "default") !== filterPresetId
      )
        return false;

      // 3. Min Score filter
      if (filterMinScore !== "0" && Number(r.overall) < Number(filterMinScore))
        return false;
      ...
      {importProgress && (
        <div className="bg-imdb-dark border border-imdb-border rounded-xl p-4 flex flex-col gap-2">
          <div className="flex justify-between text-sm text-gray-300">
            ...
            <span>
              {Math.round(importProgress.percent)}% ({importProgress.count}{" "}
              imported)
            </span>
          </div>
          <div className="w-full bg-imdb-darker rounded-full h-2.5">
            <div
              className="bg-imdb-yellow h-2.5 rounded-full transition-all duration-300"
      ...
      {filteredRatings.length === 0 ? (
        <div className="text-center py-20 bg-imdb-dark rounded-xl border border-imdb-border">
          ...
          <p className="text-gray-500 mt-2">
            Go to IMDb and start rating movies!
          </p>
        </div>
      ) : (
            ...
            >
              ...
              <div className="flex-1 flex flex-col p-5 relative">
                ...
                <div className="flex justify-between items-start mb-4 gap-2">
                  <a
                    href={`https://www.imdb.com/title/${r.movieId}/`}
                    target="_blank"
                    rel="noreferrer"
                    className="text-lg font-bold hover:text-imdb-yellow transition-colors line-clamp-2 leading-tight"
                  >
                    {r.title}
                  </a>
                  <div className="bg-imdb-darker px-3 py-2 rounded-lg border border-imdb-border text-center shrink-0 flex flex-col items-center justify-center min-w-[64px]">
                    ...
                    <div className="text-[10px] text-imdb-yellow uppercase font-bold tracking-wider mt-1">
                      Score
                    </div>
                    {(() => {
                      ...
                      return (
                        <div
                          className={`text-[10px] font-bold mt-1 bg-[#111] px-1.5 rounded ${isPos ? "text-green-500" : "text-red-500"}`}
                          title="Difference from IMDb Rating"
                        >
                          {isPos ? "+" : ""}
                          {delta.toFixed(1)}
                        </div>
...
function LazyPoster({ rating, onUpdateRating }) {
  ...
  return (
    <div className="w-[120px] shrink-0 bg-[#222] border-r border-imdb-border relative flex items-center justify-center overflow-hidden">
      {posterUrl && !posterUrl.startsWith("data:image") ? (
        <img
          src={posterUrl}
        ...
        <Film
          className={`w-10 h-10 text-gray-600 ${loading ? "animate-pulse" : ""}`}
        />
      )}
    </div>
  );
